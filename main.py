...
# --- КЛИКАБЕЛЬНЫЙ ИНТЕРАКТИВНЫЙ HEATMAP ---
def get_hm_color(intensity, theme):
    if theme == 'dark':
...
def generate_github_heatmap_b64(daily_counts_dict, theme):
    ...
    if (end_date - start_date).days > 365: start_date = end_date - timedelta(days=365) # Ограничиваем 1 годом
    ...
    for i in range(num_days):
        ...
        x_weeks.append((curr - start_date).days // 7)
        y_days.append(curr.weekday()) # 0-Пн, 6-Вс
        colors.append(max(0.1, count / max_count) if count > 0 else 0)

    fig = Figure(figsize=(10, 2.5), dpi=200, facecolor=bg_color)
...
def _conversion_task(json_path, out_pdf, date_from, date_to, font_family, font_size, theme, show_media, show_top, show_watermark, show_charts, show_wordcloud, show_toc, show_links):
    ...
    try:
        ...
        for msg in all_messages:
            ...
            try:
                ...
                if show_wordcloud: all_text_for_cloud += parse_tg_text(msg.get('text', '')) + " "
            except: pass
        ...
        /* HTML Heatmap CSS */
        .heatmap-container {{ display: flex; flex-direction: column; gap: 20px; align-items: center; margin-top: 10px; }}
        .hm-year-block {{ background: {bg_msg}; padding: 15px 20px; border-radius: 12px; border: 1px solid {border_msg}; display: inline-block; box-sizing: border-box; }}
        .hm-year-title {{ font-weight: bold; margin-bottom: 10px; color: {color_accent}; text-align: left; font-size: 1.1em; }}
        .hm-grid {{ display: flex; gap: 4px; justify-content: flex-start; overflow: hidden; }}
        .hm-labels {{ display: flex; flex-direction: column; justify-content: space-between; font-size: 0.65em; color: {'#888' if is_dark else '#666'}; padding-right: 8px; margin-top: 1px; height: 108px; }}
        .hm-labels span {{ height: 12px; line-height: 12px; display: block; }}
        .hm-col {{ display: flex; flex-direction: column; gap: 4px; }}
        .hm-cell {{ width: 12px; height: 12px; border-radius: 2px; display: block; text-decoration: none; border: 1px solid rgba(0,0,0,0.05); }}
        ...
        for idx, chunk in enumerate(chunks):
            vol_num = idx + 1
            eel.updateStatus("pulsing", f"Сборка HTML Тома {vol_num} из {len(chunks)}...", "#0ea5e9")()
            ...
            # --- СТР 3: Графики и Heatmap ---
            if vol_num == 1 and show_charts and (b64_charts or html_heatmap):
                posts_html.append(f"""
                <div class='full-page'>
                    ...
                    <div class='img-container'><img src='data:image/png;base64,{b64_heatmap}' style='width: 100%; margin-top: 20px;'></div>
                </div>
                """)

            # --- СТР 4: Облако слов и Топ ---
        ...
        eel.addLog(f"\n[SUCCESS] Генерация успешно завершена.\nОбщее время: {int(mins)} мин. {int(secs)} сек.")()
...
@eel.expose
def start_conversion(json_path, out_pdf, date_from, date_to, font_family, font_size, theme, show_media, show_top, show_watermark, show_charts, show_wordcloud, show_toc, show_links):
    threading.Thread(target=_conversion_task, args=(json_path, out_pdf, date_from, date_to, font_family, font_size, theme, show_media, show_top, show_watermark, show_charts, show_wordcloud, show_toc, show_links), daemon=True).start()
